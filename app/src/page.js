...
// Utility function to generate a random letter query
const randomQuery = () => {
...
export default function Home() {
  ...
  // Load random tracks on initial load
  useEffect(() => {
    const loadRandomData = async () => {
      ...
      try {
        ...
        setError('Failed to fetch random tracks. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
              ...
              <li
                ...
                onClick={() => {
                  setSelectedTrack({
                    ...
                    album: { cover_small: data.cover_small }
                  });
                  setModalOpen(false);
                }}
              >
                🎵 {track.title} ({Math.floor(track.duration / 60)}:
  ...
  return (
    <div className="h-screen flex flex-col bg-gray-50">
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          ...
          {!loading && activeTab === 'tracks' && (
            <>
              ...
              />
              {!tracks.length && !error && <p className="text-gray-500">No tracks found.</p>}
            </>
          )}
          ...
          {activeTab === 'playlists' && (
            <Playlist playlists={playlists} setPlaylists={setPlaylists} onTrackClick={handleTrackClick} />
          )}
        </main>
      </div>
      ...
      {/* Bottom Player */}
      {selectedTrack && (
        <footer className="fixed bottom-0 w-full bg-white shadow-inner p-4 z-50">
          <MusicPlayer
            track={selectedTrack}
            tracks={tracks} // Pass all tracks for random next
            favorites={favorites}
            setFavorites={setFavorites}
            onTrackChange={setSelectedTrack} // Allow MusicPlayer to change the track
          />
        </footer>
      )}

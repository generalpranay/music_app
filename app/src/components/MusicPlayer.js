...
export default function MusicPlayer({ track, tracks = [], favorites = [], setFavorites, onTrackChange }) {
  ...
  // Keep a simple history to implement "previous track"
  ...
  // Play a random track from the track list
  const playRandomNext = () => {
    if (!tracks.length) return;
    let nextTrack;
    do {
      nextTrack = tracks[Math.floor(Math.random() * tracks.length)];
    } while (nextTrack.id === track.id && tracks.length > 1);
  ...
  useEffect(() => {
    ...
    // Pause and clean up previous audio
    if (audioRef.current) {
    ...
    const onEnded = () => playRandomNext(); // Automatically play next random track
    ...
    <div className="fixed bottom-0 left-0 w-full bg-gray-900 text-white px-6 py-3 flex items-center justify-between border-t border-gray-800">
      ...
      <div className="flex items-center gap-3 w-1/4">
        ...
        <button
          ...
          className={`ml-2 text-xl ${isFavorite ? 'text-green-500' : 'text-gray-400 hover:text-green-400'}`}
        >
          {isFavorite ? '♥' : '♡'}
        </button>
      </div>
      ...
      <div className="flex flex-col items-center w-2/4">
        <div className="flex items-center gap-6 mb-1">
          ...
          <button onClick={playRandomNext} className="hover:scale-110 transition">
